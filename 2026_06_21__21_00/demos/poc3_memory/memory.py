...
def main() -> None:
    ...
    print(f"  recuperadas: {[m.id for m in r1]}")
    ...
    r2 = mem.retrieve(query="qual a política de garantia da geladeira?", user=U, tenant=T)
    print(f"  recuperadas: {[m.id for m in r2]} (esperado: poucas/nenhuma)\n")
    ...
    r3 = mem.retrieve(query="qual o endereço de entrega do cliente?", user=U, tenant=T)
    print(f"  recuperadas: {[m.id for m in r3]}")
    print(f"  resposta: {_responder(client, r3, 'Qual o endereço de entrega?')}")
    print("  (o endereço antigo foi marcado como 'invalidated' e não é mais recuperado)\n")
    ...
    print(f"Trilha de auditoria: {len(mem.audit)} operações registradas.")
