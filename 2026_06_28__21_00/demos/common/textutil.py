...
def mmr(
    ...
    *,
    ...
    """Maximal Marginal Relevance: seleção relevante E diversa.

    A cada passo escolhe o candidato que maximiza
        score = λ · sim(query, i) − (1 − λ) · max_{j ∈ selecionados} sim(i, j)
    Evita encher o contexto com quase-duplicatas. λ alto privilegia relevância; λ baixo
    privilegia diversidade. Retorna índices na ordem escolhida.

    Referência: Carbonell & Goldstein, "The Use of MMR..." (SIGIR 1998).
    """
    if not cand_vecs:
        return []
    rel = [cosine(query_vec, v) for v in cand_vecs]
    selected: list[int] = []
    remaining = list(range(len(cand_vecs)))
    while remaining and len(selected) < top_n:
        best_idx, best_score = remaining[0], -1e9
        for i in remaining:
            div = max((cosine(cand_vecs[i], cand_vecs[j]) for j in selected), default=0.0)
            score = lambda_ * rel[i] - (1.0 - lambda_) * div
            if score > best_score:
